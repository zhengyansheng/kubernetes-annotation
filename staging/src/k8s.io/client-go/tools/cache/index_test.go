...
func TestGetIndexFuncValuesSample(t *testing.T) {
	index := NewIndexer(MetaNamespaceKeyFunc, Indexers{"default": testIndexFunc})
	
	pod1 := &v1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "pod1", Labels: map[string]string{"foo": "bar"}}}
	pod2 := &v1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "pod2", Labels: map[string]string{"foo": "bar"}}}
	pod3 := &v1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "pod3", Labels: map[string]string{"foo": "biz"}}}
	
	index.Add(pod1)
	index.Add(pod2)
	index.Add(pod3)
	
	keys := index.ListIndexFuncValues("default")
	for _, key := range keys {
		t.Logf("key: %v", key)
	}
	//if len(keys) != 2 {
